...
class StoriesViewManagecategory extends JViewLegacy
{
	...
	protected function addBreadcrumbs(){
		...
		if ( $this->item->id )
			$pathway->addItem($this->item->name, StoriesHelperRoute::getManagecategoryRoute($this->item->id) );
		else 
			$pathway->addItem(JText::_("COM_STORIES_CATEGORY_NEW"), StoriesHelperRoute::getManagecategoryRoute() );
	}
}